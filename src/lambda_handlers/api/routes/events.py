...
async def create_event(
    payload: Dict[str, Any], tenant_id: str = Depends(verify_api_key)
):
    """
    ...
    return EventResponse(id=event_id, created_at=timestamp, status="undelivered")
...
async def get_event(event_id: str, tenant_id: str = Depends(verify_api_key)):
    """
    ...
    """
    try:
        response = events_table.get_item(Key={"pk": tenant_id, "sk": event_id})
...
async def acknowledge_event(event_id: str, tenant_id: str = Depends(verify_api_key)):
    """
...
async def delete_event(event_id: str, tenant_id: str = Depends(verify_api_key)):
    """
    Delete an event.
    ...
    try:
        ...
        # Delete the event
        events_table.delete_item(Key={"pk": tenant_id, "sk": event_id})
